...
function handleClick(event) {
	...
	//let = listaDeClasses = btn.classList;
	//for (var i = 0; i < listaDeClasses.length; i++) {
		//var classe = listaDeClasses[i];
		//console.log(classe);		
	//}	
	if (btn.classList.contains('novo')) {
		form.reset();
		form.action = 'usuarios/insert';		
		
		if(!senhaInput.hasAttribute('required')) {
			senhaInput.setAttribute('required', 'required');
		}		
	} else if(btn.classList.contains('editar')) {
		const tr             = btn.closest('tr');
		let nome             = tr.cells[0].innerText;
		let email            = tr.cells[1].innerText;		
        const dataInfo       = tr.dataset.info;
        const info           = JSON.parse(dataInfo);
        const id             = info.id;
        const dataNascimento = info.nascimento;
...
function Delete(event) {
	...
	const dataInfo  = trElement.dataset.info;
	const info      = JSON.parse(dataInfo);
	const id        = info.id;	
